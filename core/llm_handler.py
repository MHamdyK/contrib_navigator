...
def get_simple_issue_suggestion(
        ...
        language: str,
        target_count: int = 1,
        # Common, cost-effective model. Or use "gpt-4o-mini" / "gpt-4-turbo" if preferred & budget allows
    ...
    system_prompt = (
        ...
        f"the top {target_count} that would be most suitable for a beginner in {language}. "
        "Consider factors like clarity of the issue title and description (snippet), "
        "the presence of helpful labels (like 'good first issue', 'documentation', 'easy'), "
        "and the apparent scope of the task from the snippet. Avoid overly complex or vague issues."
    )
    user_prompt = (
        f"Here is a list of GitHub issues for the language '{language}'. "
        f"Please review them and suggest the top {target_count} issue(s) that seem most suitable for a beginner in {language}. "
    ...
    try:
        completion = client.chat.completions.create(
            ...
            top_p=top_p_val
            # No extra_body needed for standard OpenAI parameters like top_k
            # top_k is not typically used with gpt-3.5/4 if temperature or top_p is set.
        )
